...
use crate::{
    ...
    fast_danksharding::get_debug_data,
};
...
fn ntt_internal(v: &mut [Scalar], direction: NTTDir, device_id: usize) -> i32 {
    ...
    if direction == NTTDir::Fwd {
        unsafe { ntt_end2end(v.as_mut_ptr() as *mut ScalarField, v.len(), false) };
    } else if direction == NTTDir::Inv {
        unsafe { ntt_end2end(v.as_mut_ptr() as *mut ScalarField, v.len(), true) };
    }
...
fn ecntt_internal(values: &mut [Point], device_id: usize, inverse: bool) -> i32 {
    unsafe {
        ecntt_end2end(
...
mod tests {
    ...
    use crate::{
        field::{generate_random_points, BaseField, BASE_LIMBS, SCALAR_LIMBS, generate_random_points_ark, generate_random_scalars_ark},
    ...
    fn test_msm() {
        ...
        let n = 1<<2;
        let mut scalars_r = vec![];
        let mut points_r = vec![PointAffineNoInfinity::default();4];

        scalars_r.push( Scalar::from_limbs( &[3827484040, 2874625294, 4134484179, 2522098891, 1684039639, 4190761864, 1674792009, 1733172596]));
        scalars_r.push( Scalar::from_limbs( &[2859778174, 247198543, 1069683537, 986951671, 18230349, 1865405355, 3834758898, 1605705230]));
        scalars_r.push( Scalar::from_limbs( &[870702623, 2140804196, 1118323047, 4097847923, 733285949, 2517599604, 2748585063, 1465198310]));
        scalars_r.push( Scalar::from_limbs( &[2997668077, 4130616472, 4255624276, 3713720096, 813455961, 1818410993, 1796699074, 1289452986]));
      
        points_r[0].x = BaseField::from_limbs( &[ 927572523, 3123811016, 1178179586, 448270957, 3269025417, 873655910, 946685814, 846160237, 2311665546, 894701547, 1123227996, 414748152]);
        points_r[0].y = BaseField::from_limbs( &[ 2100670837, 1657590303, 4206131811, 3111559769, 3261363570, 430821050, 2016803245, 2664358421, 3132350727, 189414955, 1844185218, 11036570]);
        points_r[1].x = BaseField::from_limbs( &[ 606938594, 3862011666, 3396180143, 765820065, 3281167117, 634141057, 210831039, 670764991, 3442481388, 2417967610, 1382165347, 243748907]);
        points_r[1].y = BaseField::from_limbs( &[ 2486871565, 3199940895, 3186416593, 2451721591, 4108712975, 2604984942, 1165376591, 854454192, 1479545654, 1006124383, 1570319433, 22366661]);
        points_r[2].x = BaseField::from_limbs( &[ 183039612, 256454025, 4250922080, 2485970688, 3679755773, 1397028634, 1298805238, 3413182507, 2291846949, 1280816489, 1119750210, 122833203]);
        points_r[2].y = BaseField::from_limbs( &[ 3025851512, 1147574033, 1323495323, 569405769, 382481561, 1330634004, 3879950484, 1158208050, 2740575984, 2745897444, 3101936482, 405605297]);
        points_r[3].x = BaseField::from_limbs( &[ 4006417784, 3580973450, 2524244405, 3414509667, 4142213295, 3876406748, 4116037682, 877187559, 3606672288, 3459819278, 3198860768, 30571621]);
        points_r[3].y = BaseField::from_limbs( &[ 182896763, 2741166359, 626891178, 1601768019, 1967793394, 706302600, 2612369182, 2051460370, 2918333441, 1902350841, 475238909, 239719017]);

        
        let msm_result = msm(&points_r, &scalars_r, 0);
        
        println!("the point {:?}", msm_result);

        let point_r_ark:Vec<_> = points_r.iter().map(|x| x.to_projective().to_ark().into_affine()).collect();
        let scalars_r_ark:Vec<_> = scalars_r.iter().map(|x| x.to_ark().into_repr()).collect();
        ...
        let mut points_ark = generate_random_points_ark(n, get_rng(seed));
        let scalars_ark = generate_random_scalars_ark(n, get_rng(seed));

        let points: Vec<_> = points_ark.iter().map(|p| PointAffineNoInfinity::from_ark(p)).collect(); //generate_random_points(n, get_rng(seed));
        let scalars: Vec<_> = scalars_ark.iter().map(|s| Scalar::from_ark(s)).collect(); //generate_random_scalars(n, get_rng(seed));

        let msm_result_ark = VariableBaseMSM::multi_scalar_mul(&points_ark, &scalars_ark.iter().map(|s| s.0).collect::<Vec<_>>());
