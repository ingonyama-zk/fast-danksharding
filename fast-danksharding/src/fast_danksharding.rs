...
use crate::{
    addp_vec, get_debug_data_points_proj_xy1_vec, get_debug_data_points_xy1,
    get_debug_data_scalars,
    matrix::{interleave_cols, rows_to_cols, split_vec_to_matrix},
    utils::list_to_reverse_bit_order,
};
...
pub fn main_flow() {
    ...
    let mut d1 = split_vec_to_matrix(&d1_flat, 2 * N_ROWS).clone();
    debug_assert_eq!(d1, d1_debug);

    let count = d1.len();
    d1.iter_mut().enumerate().for_each(|(i, delta0_i)| {
        //delta0 = ECINTT_row(d1) 1x512
        iecntt(delta0_i, 0);

        debug_assert_eq!(delta0_i, &delta0_debug[i]);

        // delta1 = delta0 << 256 1x512
        let delta1 = [&delta0_i[N_ROWS..], &vec![Point::infinity(); N_ROWS]].concat();
        debug_assert_eq!(delta1, delta1_debug[i]);

        //q[mu] = ECNTT_row(delta1) 1x512
        let mut q = delta1;
        ecntt(&mut q, 0);
        debug_assert_eq!(q, q_debug[i]);
