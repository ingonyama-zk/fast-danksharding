 ...
 * @param omega multiplying factor. 
 */
 scalar_t * create_twiddle_factors_on_device(uint32_t n_twiddles, scalar_t omega) {
  size_t size_twiddles = n_twiddles * sizeof(scalar_t);
  scalar_t * d_twiddles;
  cudaMalloc( & d_twiddles, size_twiddles);
  twiddle_factors_kernel <<< 1, 1 >>> (d_twiddles, n_twiddles, omega);
 ...
 * @param inverse indicate if the result array should be normalized by n^(-1). 
 */
 extern "C" int32_t ntt_end2end(scalar_t * arr, uint32_t n, bool inverse) {
  uint32_t n_twiddles = n;
  scalar_t * d_twiddles;
  if (inverse){
    d_twiddles = create_twiddle_factors_on_device(n_twiddles, scalar_t::omega_inv());
  } else{
    d_twiddles = create_twiddle_factors_on_device(n_twiddles, scalar_t::omega());
  }
 ...
 * @param inverse indicate if the result array should be normalized by n^(-1). 
 */
 extern "C" int32_t ecntt_end2end(projective_t * arr, uint32_t n, bool inverse) {
  uint32_t n_twiddles = n;
  scalar_t * d_twiddles;
  if (inverse){
    d_twiddles = create_twiddle_factors_on_device(n_twiddles, scalar_t::omega_inv());
