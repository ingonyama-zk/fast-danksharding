...
template <class CONFIG> class Field {
  ...
  public:
    ...
    // Note: only works for p=3 (mod 4), so doesn't work for scalar fields and BLS12-377 base field
    static constexpr HOST_DEVICE_INLINE Field sqrt(const Field& xs) {
      Field res = one();
  #ifdef __CUDA_ARCH__
  #pragma unroll
  #endif
      for (int i = 0; i < NBITS; i++) {
        ...
        Field p_minus_1 = div2(get_modulus() - one());
        Field exp = div2(p_minus_1 - one()) + one();
        if (exp.get_scalar_digit(NBITS - i - 1, 1)) {
          res = res * xs;
        }
      }
      return res;
